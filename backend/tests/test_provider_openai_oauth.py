...
async def test_query_luna_uses_websocket_transport():
    ...
    with (
        ...
        patch(
            "backend.providers.openai_oauth.query_responses_lite_websocket",
...
async def test_responses_lite_websocket_collects_text():
    from backend.oauth.responses_websocket import query_responses_lite_websocket
    ...
    with patch("backend.oauth.responses_websocket.websockets.connect", return_value=fake):
        result = await query_responses_lite_websocket(
    ...
    assert '"type": "response.create"' in fake.sent[0] or '"type":"response.create"' in fake.sent[0]
    assert "x-openai-internal-codex-responses-lite" not in fake.sent[0]
